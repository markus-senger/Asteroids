...
private:
	...
	std::vector<std::unique_ptr<asteroid>>		m_new_asteroids;
	std::vector<std::unique_ptr<enemy_ship>>	m_enemies;
	size_t		m_hits			{ 0 };
	size_t		m_points		{ 0 };
	...
	void check_collision_asteroids_asteroids		(std::unique_ptr<asteroid>& asteroid_obj);
	void check_collision_asteroids_rockets			(std::unique_ptr<asteroid>& asteroid_obj);
	void check_collision_asteroids_enemy_ships		(std::unique_ptr<asteroid>& asteroid_obj);
	void check_collision_rockets_enemy_ships		(std::unique_ptr<rocket>& rocket_obj);
	void check_collision_enemy_ships_other_ships	(std::unique_ptr<enemy_ship>& enemy_obj);
